...
const DoctorCard = ({ doctor, compact = false }: DoctorCardProps) => {
  ...
  // Get available days from doctor's schedule - more compact for mobile
  const getAvailableDaysCount = () => {
    if (!doctor.schedule) return 0;
  ...
  const availableDaysCount = getAvailableDaysCount();

  // Completely redesigned compact doctor card (mobile optimized)
  if (compact) {
    return (
      <Card className="overflow-hidden border border-gray-200 hover:border-brand/30 transition-colors rounded-xl shadow-sm hover:shadow-md">
        <div className="relative">
          ...
          <div className="p-4">
            ...
            <div className="flex items-center gap-3">
              <Avatar className="h-14 w-14 rounded-full border-2 border-gray-100">
                {doctor.image ? (
              ...
              <div className="flex-grow">
                ...
                <h3 className="font-bold text-base text-gray-800">{doctor.name}</h3>
              </div>
            </div>
            
            {/* Doctor fees - more compact */}
            <div className="mt-3 flex justify-between text-xs text-gray-500">
              <span>{formatFee(doctor.fees.examination)} للكشف</span>
              {availableDaysCount > 0 && (
                <span className="text-brand">{availableDaysCount} أيام متاحة</span>
              )}
            </div>
            ...
            <div className="mt-3 flex gap-2">
              <Button 
                variant="outline" 
                className="text-xs p-2 h-9 flex-1 text-brand border-brand/30 hover:bg-brand/5"
                onClick={() => setShowDialog(true)}
              >
                التفاصيل
              </Button>
              <Button 
                className="text-xs p-2 h-9 flex-1 bg-green-600 hover:bg-green-700 text-white"
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="p-0 sm:max-w-lg mx-auto rounded-lg overflow-hidden max-h-[90vh]">
            <DoctorDetails 
              doctor={doctor} 
  ...
  // Full sized doctor card - also improved for mobile
  return (
    <motion.div 
    ...
    >
      <div className="flex flex-col md:flex-row">
        <div className="md:w-1/3 bg-gradient-to-br from-brand/5 to-brand/10 p-4 md:p-6 flex items-center justify-center">
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-white shadow p-1.5 flex items-center justify-center overflow-hidden">
            {doctor.image ? (
              ...
              <div className="w-full h-full rounded-full bg-brand/10 flex items-center justify-center">
                <User className="h-12 w-12 md:h-16 md:w-16 text-brand/40" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="md:w-2/3 p-4 md:p-6">
          ...
          {doctor.bio && (
            <p className="text-gray-600 mb-3 text-sm md:text-base line-clamp-2">{doctor.bio}</p>
          )}
          
          <div className="grid grid-cols-2 gap-2 md:gap-3 mb-3 md:mb-4">
            <div className="bg-gray-50 rounded p-2 flex items-center text-sm">
              <Calendar className="h-4 w-4 text-brand ml-2 flex-shrink-0" />
              <div className="overflow-hidden">
                <span className="text-xs text-gray-500 block">رسوم الكشف</span>
                <p className="font-semibold truncate">{formatFee(doctor.fees.examination)}</p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded p-2 flex items-center text-sm">
              <Calendar className="h-4 w-4 text-brand ml-2 flex-shrink-0" />
              <div className="overflow-hidden">
                <span className="text-xs text-gray-500 block">الاستشارة</span>
                <p className="font-semibold truncate">{formatFee(doctor.fees.consultation || 'غير متاح')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-3 flex items-center">
            ...
            <span className="text-sm text-gray-700">
              {availableDaysCount > 0 ? `متاح ${availableDaysCount} أيام في الأسبوع` : 'يرجى الاتصال لمعرفة المواعيد'}
            </span>
          </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="p-0 max-w-[95%] sm:max-w-md md:max-w-lg mx-auto rounded-lg overflow-hidden max-h-[90vh]">
          <DoctorDetails 
            doctor={doctor} 
            onBookingWizard={redirectToBookingWizard} 
            onWhatsApp={openWhatsApp} 
...
const DoctorDetails = ({ 
  ...
  return (
    <div className="bg-white overflow-hidden max-h-[85vh] flex flex-col">
      ...
      {/* Schedule section - Simplified for mobile */}
      <div className="border-t mx-4 pt-3 pb-4">
        ...
        <div className="max-h-48 overflow-y-auto">
          {availableDays.length > 0 ? (
            <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 px-1">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Array.isArray(times) && times.length > 0 && (
                        <span className="bg-brand/10 text-brand px-2 py-0.5 rounded-full text-xs">
                          {times[0]} {times.length > 1 ? `+${times.length - 1}` : ''}
                        </span>
                      )}
                    </div>
                  </div>
